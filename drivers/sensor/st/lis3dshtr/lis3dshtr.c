...
static int lis3dshtr_write_register(const struct device *dev, uint8_t reg, uint16_t value) {
...
static int lis3dshtr_read_register(const struct device *dev, uint8_t reg, uint8_t *val)  {
	...
	ret = spi_tranceive_dt(&cfg->spi_controller, &tx_set, &rx_set);
...
static int lis3dshtr_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	ret = lis3dshtr_read_register(&config->spi_controller, 0x29, data->sample_int);
	...
	ret = lis3dshtr_read_register(&config->spi_controller, 0x28, data->sample_int);
...
static int lis3dshtr_channel_get(const struct device *dev, enum sensor_channel chan, struct sensor_val *val)
{
	struct st_lis3dshtr_data *data = dev->data;
	int16_t raw;
	int32_t micro_g;
